...
.main-content-area-wrapper {
    ...
    position: relative; /* Untuk positioning social-sidebar */
}
...
.social-sidebar {
    ...
    gap: 15px;
    border-top-right-radius: 20px; /* Sudut kanan atas */
    border-bottom-right-radius: 20px; /* Sudut kanan bawah */
    position: absolute; /* Posisi absolut di dalam wrapper */
    right: 0; /* Taruh di paling kanan */
    top: 0;
    bottom: 0;
    z-index: 500; /* Pastikan di atas konten */
}
...
.social-sidebar a.instagram { background-image: linear-gradient(45deg, #f09433 0%, #e6683c 25%, #dc2743 50%, #cc2366 75%, #bc1888 100%); }
...
.organogram {
    display: flex;
    flex-direction: column; /* Mengubah ke kolom untuk menampilkan Masyur di atas Senat di bawah */
    justify-content: center;
    align-items: center; /* Pusatkan bagan secara horizontal */
    gap: 40px; /* Jarak antar dua bagan utama (Masyur dan Senat) */
    margin-top: 30px;
    padding: 20px;
    background: #f8faff;
    border-radius: 15px;
...
.sub-organization-container {
    ...
    flex-direction: column;
    align-items: center;
    gap: 0; /* Jarak antar level akan diatur oleh margin dan garis */
    padding: 20px;
    border: 1px dashed #cce7ff; /* Border putus-putus untuk membedakan */
    border-radius: 15px;
    background: rgba(255, 255, 255, 0.7);
    box-shadow: 0 2px 8px rgba(0,0,0,0.05);
    width: 100%; /* Ambil lebar penuh dari parent organogram */
    max-width: 700px; /* Batasi lebar agar tidak terlalu melebar */
    min-width: 300px; /* Minimum width untuk setiap bagan */
}

.sub-organization-container h4.sub-org-title {
    font-size: 22px; /* Disesuaikan agar tidak terlalu besar */
    color: #0b6cb3;
    margin-bottom: 15px;
    font-weight: 700;
    text-align: center;
...
.org-level {
    ...
    flex-wrap: wrap; /* Izinkan wrap untuk kartu dalam level yang sama */
    gap: 20px; /* Jarak antar kartu dalam satu level */
    width: 100%; /* Lebar penuh di dalam container sub-organisasi */
    position: relative; /* Untuk pseudo-elemen garis */
    margin-top: 40px; /* Jarak antar level, cukup ruang untuk garis */
}

.org-level:first-of-type {
    margin-top: 0; /* Level paling atas tidak perlu margin-top */
}
...
.org-card {
    ...
    min-width: 160px; /* Sesuaikan min-width */
    position: relative; /* Untuk pseudo-elemen garis */
    flex-basis: auto; /* Biarkan browser menentukan lebar berdasarkan konten */
    max-width: 220px; /* Maksimal lebar untuk kartu */
}
...
.org-card.leader { /* Ketua dan Wakil */
    max-width: 200px; /* Maksimal lebar untuk kartu leader */
}
...
.org-level::before { /* Garis horizontal untuk semua level kecuali yang pertama */
    content: '';
    position: absolute;
    top: -20px; /* Posisi vertikal garis horizontal */
    left: 0;
    right: 0;
    height: 2px;
    background-color: #a7d9f7;
    z-index: 1;
    width: calc(100% - 40px); /* Sesuaikan dengan padding horizontal .org-level */
    margin: 0 auto; /* Pusatkan garis horizontal */
}
...
.org-level .org-card::before {
    content: '';
    position: absolute;
    top: -20px; /* Posisi vertikal garis pendek dari horizontal ke kartu */
    left: 50%;
    transform: translateX(-50%);
    width: 2px;
    height: 20px;
...
.org-level.sekben {
    ...
    gap: 40px; /* Jarak yang lebih lebar antara Sekre dan Bendahara */
    width: 100%;
}
.org-level.sekben .org-card {
    flex-basis: calc(50% - 20px); /* 2 kartu per baris, dengan gap 40px */
    max-width: 200px; /* Maksimal lebar untuk kartu Sekben */
    min-width: 150px;
}
...
.org-level.divisi {
    display: flex;
    flex-wrap: wrap; /* Izinkan wrapping agar tidak ada scroll */
    justify-content: center; /* Pusatkan */
    gap: 15px; /* Jarak antar divisi */
    width: 100%;
    overflow-x: visible; /* Pastikan tidak ada scroll */
    padding-bottom: 0; /* Hapus padding scrollbar */
}
.org-level.divisi .org-card {
    flex-shrink: 1; /* Izinkan kartu mengecil jika perlu */
    flex-grow: 1; /* Izinkan kartu tumbuh jika ada ruang */
    flex-basis: calc(33.333% - 10px); /* Contoh: 3 kartu per baris */
    max-width: 180px; /* Batasi maksimal lebar per kartu divisi */
    min-width: 150px; /* Jaga agar tidak terlalu kecil */
}
...
@media (max-width: 1200px) {
    ...
    .content-section {
        width: calc(100% - 60px); /* Pertahankan lebar konten untuk sidebar */
    ...
    .sub-organization-container {
        max-width: 600px; /* Sedikit perkecil max-width agar lebih rapi di layar menengah */
    }
    .org-level.divisi .org-card {
        flex-basis: calc(50% - 10px); /* 2 kartu per baris di layar menengah */
    }
}
...
@media (max-width: 992px) {
    ...
    .main-content-area-wrapper {
        ...
        border-radius: 0;
        box-shadow: none;
        position: relative; /* Kembali ke relatif agar sidebar bisa di dalam */
    }
    .social-sidebar {
        width: 100%;
        height: auto;
        ...
        border-radius: 0;
        box-shadow: 0 2px 5px rgba(0,0,0,0.1);
        position: static; /* Kembali ke static atau relatif agar tidak tumpang tindih */
        order: 1; /* Posisikan di atas konten utama jika flex-direction column */
    }
    .social-sidebar .sidebar-bg {
        border-radius: 0;
    }
    ...
    .content-section {
        ...
        order: 2; /* Posisikan di bawah sidebar */
    }
    .content-section h2.page-title {
        font-size: 32px;
    }
    ...
    .sub-organization-container {
        flex-basis: 100%;
        max-width: 90%; /* Kontainer bagan akan memenuhi lebar */
    }
    ...
    .org-level.sekben .org-card {
        flex-basis: calc(50% - 20px); /* Tetap 2 kartu */
        max-width: 200px;
    }
    .org-level.divisi .org-card {
        flex-basis: calc(50% - 10px); /* 2 kartu per baris di mobile */
    }
    .org-level {
        flex-direction: row; /* Kembali ke baris untuk kartu dalam satu level */
        flex-wrap: wrap; /* Izinkan wrapping */
        align-items: flex-start; /* Sejajarkan ke atas */
        justify-content: center;
        gap: 15px; /* Jarak antar kartu dalam satu level */
    }

    /* Garis responsif */
    .org-card.leader::after { /* Garis vertikal di bawah leader di mobile */
        content: '';
        position: absolute;
        bottom: -20px; /* Jarak ke bawah */
        left: 50%;
    ...
    .org-level::before { /* Garis horizontal menjadi vertikal panjang di tengah */
        left: 50%;
        right: auto;
        transform: translateX(-50%);
        width: 2px;
        height: 40px; /* Tinggi untuk konektor antar level */
        background-color: #a7d9f7;
        top: -40px; /* Posisikan lebih tinggi */
    }

    .org-level:first-of-type::before {
        display: none; /* Sembunyikan garis di atas level pertama */
    }

    .org-level .org-card::before { /* Garis pendek dari garis vertikal pusat ke kartu */
        content: '';
        position: absolute;
        top: -20px;
        left: 50%;
...
@media (max-width: 768px) {
    ...
    .org-card h4 {
        font-size: 16px; /* Disesuaikan lagi untuk layar lebih kecil */
    }
    .org-card p {
        font-size: 12px; /* Disesuaikan lagi untuk layar lebih kecil */
    }
    .sub-organization-container h4.sub-org-title {
        font-size: 20px; /* Judul sub-organisasi juga disesuaikan */
    }
    .org-level.divisi .org-card {
        flex-basis: calc(50% - 10px); /* Tetap 2 kartu per baris */
    }
}
...
@media (max-width: 480px) {
    ...
    .org-card {
        padding: 10px 12px; /* Kurangi padding lagi */
    }
    .org-card h4 {
        font-size: 15px; /* Lebih kecil lagi */
    }
    .org-card p {
        font-size: 11px; /* Lebih kecil lagi */
    }
    .sub-organization-container h4.sub-org-title {
        font-size: 18px; /* Judul sub-organisasi juga lebih kecil */
    }
    .org-level.divisi .org-card {
        flex-basis: 100%; /* Satu kartu per baris di layar sangat kecil */
        max-width: 250px; /* Batasi lebar kartu di layar sangat kecil */
    }
}
</style>
</head>
